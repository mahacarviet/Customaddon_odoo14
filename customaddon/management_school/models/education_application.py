...
class StudentApplication(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('application_no', _('Mới')) == _('Mới'):
            vals['application_no'] = self.env['ir.sequence'].next_by_code(
                'education.application') or _('Mới')
    ...
    prev_school_distrist = fields.Char(string='Quận/Huyện')
    prev_school_city = fields.Char(string='Thành phố')
    prev_school_note = fields.Char(string='Các thành tích nổi bật đạt được')
    image = fields.Image(string='Ảnh', attachment=True)
    academic_year_id = fields.Many2one(
        'education.academic.year', string='Năm học',
        help="Người dùng chọn năm học đăng ký học cho sinh viên")
    language_speaking = fields.Char(string="Ngôn ngữ gốc", default='Việt Nam')
    ...
    application_no = fields.Char(string='Mã đơn đăng ký', required=True,
                                 ...
                                 index=True, default=lambda self: _('Mới'))
